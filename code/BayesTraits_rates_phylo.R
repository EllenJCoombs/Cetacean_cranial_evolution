...
#This is where you pull in your rates from BayesTraits (here, I am looking at the whole whale skull - you could also just put in the rates for the nasal or premax etc
...
#Create a colour palette for the rates
...
p<-ggtree(tree_2_w_data, aes(color = log(meanRate)), size=2)+ #this is where you can add  layout="fan" if you want a fan 
  scale_colour_stepsn(breaks = seq(from=-1,to=6,by=.2), colours = colspal(100))+ #colspal is how many cols you want (the range)
  #theme(legend.position="top")+ #play around with the legend
